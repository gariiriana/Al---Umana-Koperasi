...
export function MbgPurchasingPage() {
  ...
  useEffect(() => {
    if (!selectedBatchId || loading || orders.length > 0) return;
    if (adjustedRecipeRequirements.length > 0 && autoGeneratedBatchRef.current !== selectedBatchId) {
            ...
            <>
              ...
              {filteredOrders.length === 0 ? (
                <div className="bg-white border border-[#E5E7EB] rounded-2xl p-12 text-center shadow-xs">
                  <Wand2 className="mx-auto h-12 w-12 text-[#FBBF24] mb-3 animate-pulse" />
                  <h3 className="text-lg font-extrabold text-[#111827]">Generate Daftar Belanja MBG</h3>
                  <p className="text-sm text-gray-500 mt-1 max-w-md mx-auto">
                    Kebutuhan bahan makanan siap dikalkulasikan secara otomatis berdasarkan resep dan porsi dari Tim Produksi.
                  </p>
                  <button
                    onClick={() => handleGenerateNewPoFromProduction()}
                    className="mt-5 inline-flex items-center gap-2 bg-[#059669] hover:bg-[#047857] text-white font-extrabold text-xs px-5 py-3 rounded-xl cursor-pointer shadow-md active:scale-95 transition-all"
                  >
                    <Wand2 className="h-4 w-4 text-[#FBBF24]" />
                    <span>✨ Generate Otomatis Daftar Belanja dari Produksi</span>
                  </button>
                </div>
              ) : viewMode === 'table' ? (
