...
// Enable offline persistence safely for Firestore to handle bad signal / offline edits
  ...
  try {
    enableIndexedDbPersistence(db).catch((err) => {
      if (err.code === 'failed-precondition') {
        ...
        console.warn('[firebase] Firestore persistence unimplemented in this browser');
      } else {
        console.warn('[firebase] Firestore persistence error:', err);
      }
    });
  } catch (err) {
    console.warn('[firebase] Firestore persistence synchronous error:', err);
  }
}
